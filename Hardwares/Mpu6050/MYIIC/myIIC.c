...
{
	uint8_t r_value = 0;
    r_value = HAL_I2C_Mem_Read(&hi2c2, dev_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, pdata, length, 1000);
	return r_value;
}
uint8_t IIC_WriteBytes(uint8_t dev_addr,uint8_t reg_addr, uint8_t length, uint8_t *pdata)
{
	uint8_t r_value = 0;
    r_value = HAL_I2C_Mem_Write(&hi2c2, dev_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, pdata, length, 1000);
	return r_value;
}
/**************************实现函数********************************************
...
{
	uint8_t r_value = 0;
	r_value = HAL_I2C_Mem_Read(&hi2c2, dev_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, data, 1, 1000);
	return r_value;	
}
uint8_t IIC_WriteByte(uint8_t dev_addr, uint8_t reg_addr, uint8_t data)
{
	uint8_t r_value = 0;
    r_value = HAL_I2C_Mem_Write(&hi2c2, dev_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, &data, 1, 1000);
	return r_value;
}
